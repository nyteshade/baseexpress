...
var combiner = function(options){
  ...
  var requireMatch  = /require\([',"]?(.*?)[',"]?\);?/gi,
      ...
      vars          = config.vars,
      combined      = "",
      comboFileName = config.outputName || "combined",
      promise       = Q.defer();

  //run through all files from config
  config.files.forEach(function (file, index, array) {
    ...
    fs.readFile(fullFilePath, 'utf8', function (err, data) {
      if (err) {
        log('Error retrieving file: ' + file + ' from ' + fullFilePath); 
      } 
      else {
        //replace all requires with file contents         
        data = data.replace(requireMatch, function(pattern, path){
          var fullPath  = _path.join(config.cwd, config.folder, path);
          ...
          try {
            ...
            return '/* ' + pattern + ' cannot be retrieved */';
          }
        });
  ...
  //finished
  promise.promise.done(function(combined) {
    //build output name
    if (_path.extname(comboFileName) === '') {
      comboFileName = [
        comboFileName, '.',
        config.packedSuffix, '.js'
      ].join('');       
    }

    //write combined file
    fs.writeFile(_path.join(config.cwd, config.folder, comboFileName), combined, function (err) {
      if (err){
        log('There was error writing a file: ' + comboFileName);
      } else {
        log('Writing file :' + comboFileName);
      }
    }); 
  });
