  ...
  readFiles: function(files, cache) {
    ...
    this.ROOTS.forEach(function(root, rootIndex, roots) {
      files.forEach(function(file, fileIndex) {
        ...
        missing = false;
        if ((exists = fs.existsSync(path)) && !cache[path]) {
          stat = fs.statSync(path);
          if (!cache[path] || cache[path].stat.mtime.getTime() <= 
              stat.mtime.getTime()) {
            cache[path] = {
          ...
          reqs = Combiner.getRequired(cache[path].content);
          if (reqs.length) {
            cache = self.readFiles(reqs, cache);
            self.requirements.concat(reqs);
          }
          cache['output'] += cache[path].content;
        }
        else {
          if (!exists) {
            err('Cannot locate %s', path);
          }
          else {
            state.dev 
                && log('\x1b[92mSKIP\x1b[90m %s (Already Processed)\x1b[0m', path);
          }
        }
      });
    });
